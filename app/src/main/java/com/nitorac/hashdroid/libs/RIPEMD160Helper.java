...
public class RIPEMD160Helper {
    ...
    public void update(byte[] input) {
        for (byte anInput : input) {
            working[working_ptr >> 2] ^= ((int) anInput) << ((working_ptr & 3) << 3);
            working_ptr++;
            if (working_ptr == 64) {
                compress(working);
                for (int j = 0; j < 16; j++) {
    ...
    public static String cryptRIPEMD160(String txt) {
        RIPEMD160Helper r = new RIPEMD160Helper();
        r.update(txt);
        return r.digest();
    }
}